...
import { createLesson, updateLesson } from '@/actions/admin.action';
...
const CreateLessonModal = ({ children, lessonId, lessonData }: CreateLessonModalProps) => {
	...
	async function onSubmit(values: z.infer<typeof lessonSchema>) {
		...
		try {
			if (lessonId) {
				await updateLesson(lessonId, { course: courseId, sectionId, ...values });
			} else {
				await createLesson({ course: courseId, sectionId, ...values });
			}
