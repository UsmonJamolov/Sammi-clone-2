...
const LessonList = ({lessons}: LessonListProps) => {
    const onDragEnd = (result: DropResul) => {
        ...
        const updateLessons = items.splice(startIndex, endIndex + 1)

        const data = updatedLesson.map(s => ({
            _id: s._id,
...
function LessonCard({lesson, index}: {lesson: LessonType, index: number}) {
    ...
    return (
        <Draggable draggable={lesson._id} index={index}>
            {provied => (
                <div 
                className="flex items-center gap-x-2 rounded-md text-sm border-bg-secodary group relative"
                ref={provided.innerRef}
                {...provided.draggableProps}
                >
                    {loading && (
                        <div className="absolute inset-0 bg-backgorund/70 flex items-center justify-center rounded-md z-10 animate-pluse">
                            <Spinner />
                        </div>
                    )}
                    <div className="rounded-l-md border-r border-r-foreground/30 px-2 py-3 transition-all hover:bg-foreground/10 cursor-pointer" {...provided.dragHandleProps}>
                        <Grip />
                    </div>
                    <span>{lesson.title}</span>
                    <div className="ml-auto mr-2 apacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-x-1">
